...
async function protege(usuario) {
  if (tieneRol(usuario, ["Superadmin", "Alumno"])) 
  { 
      usuario = usuario.email;
      const user = await daoUsuario.doc(usuario).get();
      const datau = user.data(); //datau.matricula
      var datau_matricula = datau.matricula;
      consulta(datau_matricula);
  }
}

async function consulta(usuario) {
  const doc = await daoPersonal.doc(usuario).get();
  let html = "";
  if(doc.exists)
  {
      html += await htmlFila(doc);
...
async function htmlFila(doc) {
  ...
  {
      var datos = '<tr><td>'+nombre+'</td><td>'+materia[x]+'</td><td>'+calificacion[x]+'</td></tr>';
      if(status == "Alumno" && materia != [])
